...
select distinct c.customer_id, c.first_name, p.category
from customers c
...
select p.product_name, sum(s.quantity_sold * p.price) as total_sales
from products p
...
select distinct c.customer_id, c.first_name, s.sale_date
from customers c
join sales s on c.customer_id = s.customer_id
where extract(year from s.sale_date) = 2023;

-- 24. Write a query to find the customer with the highest total sales in 2023.
select c.customer_id, c.first_name, sum(s.quantity_sold * p.price) as total_sales
from customers c
join sales s on c.customer_id = s.customer_id
join products p on s.product_id = p.product_id
where extract(year from s.sale_date) = 2023
...
from sales s
join products p on s.product_id = p.product_id
where (s.quantity_sold * p.price) > 500;
...
select c.customer_id, c.first_name, sum(s.quantity_sold) as total_products
from customers c
join sales s on c.customer_id = s.customer_id
group by c.customer_id, c.first_name
having sum(s.quantity_sold) > 5;
...
select c.customer_id, c.first_name, sum(s.quantity_sold * p.price) as total_spent
from customers c
join sales s on c.customer_id = s.customer_id
join products p on s.product_id = p.product_id
where extract(year from s.sale_date) = 2023
...
from products
where stock_quantity = (select lect max(stock_quantity) from products)
...
select first_name, last_name, p.product_name, sum(s.quantity_sold * p.price) as total_sales
from customers c
join sales s
on c.customer_id = s.customer_id
join products p
on s.product_id = p.product_id
where c.membership_status = 'Gold'
group by first_name,last_name, p.product_name;

-- 42. Write a query to find the total sales of products by category.
select p.category, sum(s.quantity_sold * p.price) as total_sales
from products p
join sales s
on p.product_id = s.product_id
group by p.category;
...
select p.product_name, price, sum(s.quantity_sold) as total_quantity
from products p
join sales s
on p.product_id = s.product_id
where p.price between 200 and 800
group by p.product_name, price;
